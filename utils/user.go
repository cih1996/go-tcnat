...
func NewUser(serverAddr string, serverPort int, transferPort int, tagID string, localServerAddr string) (*User, error) {
...
func (u *User) handleTransfer(clientTag string) {
	...
	conn, err := net.Dial("tcp", u.serverAddr+":"+strconv.Itoa(u.transferPort))
	if err != nil {
